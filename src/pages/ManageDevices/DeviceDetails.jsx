...
const DeviceDetails = () => {
  ...
  const System_app_list = deviceData?.application.filter((app) => app.is_system_app);
  const Non_System_app_list = deviceData?.application.filter((app) => !app.is_system_app);
  ...
  const rebootDevice = async (device_id) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/api/reboot_device`, { device_id });
...
const handelUninstallApp = async (package_name) => {
    try{
      const response =await axios.post(`${API_BASE_URL}/api/uninstall_app`, { "device_id":deviceData.device_id, "package_name":package_name });
      if(response.data.status){
      ...
      toast.error("Error uninstalling app");
    }
  }

  const handelClearAppData = async (package_name) => {
    try{
      const response =await axios.post(`${API_BASE_URL}/api/clear_app_data`, { "device_id":deviceData.device_id, "package_name":package_name });
      if(response.data.status){
      ...
      toast.error("Error clearing app data");
    }
  }
      ...
      {apps.map((app, idx) => (
        ...
        >
          ...
          <div className="flex-1">
            <h3 className="text-base font-semibold truncate">{app.app_name}</h3>
            <p className="text-sm text-gray-500 truncate">{app.app_version}</p>
            <p className="text-sm text-gray-400 truncate">{app.package_name}</p>
          </div>
          {!app.is_system_app && (
            <DropdownMenu>
  ...
  return (
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="p-6 space-y-8">
        <AppList title="Installed Applications" apps={Non_System_app_list} icon={androidLogo} />
        <AppList title="System Applications" apps={System_app_list} icon={systemAppLogo} />
      </div>
    </div>
  );
};
